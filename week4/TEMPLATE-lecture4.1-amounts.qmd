...
jobs_clean %>% 
  filter(year == 2016) %>% 
  slice_max(order_by = total_earnings, n = 10) %>% # re-order by total earnings column and take top ten rows. Top ten highest occupations
  mutate(occupation = fct_reorder(.f = occupation, .x = total_earnings)) |> # Orders occupation by total earnings 
  ggplot(aes(x = occupation, y = total_earnings)) +
  geom_col() + # because you have a y var
  geom_text(aes(label = scales::dollar(total_earnings)), hjust = 1.2, color = "white") + #hjust adjusts l/r buffer
...
jobs_clean |> 
  ...
  ggplot(aes(x = occupation, y = total_earnings)) +
  geom_point() +
  geom_linerange(aes(ymin = 0, ymax = total_earnings)) +# draw lines, the stick of the lollipip, geom_segment also works
  geom_text(aes(label = scales::dollar(total_earnings)), hjust = -0.2) + 
  scale_y_continuous(labels = scales::label_currency(accuracy = 1, scale = 0.001, suffix = "k"), limits = c(0, 250000)) + # expand axis to make room for values
  coord_flip()
...
jobs_clean |>
  ...
  coord_flip()
# have to dodge points and lines. Lines and points don't have an inherent width, so you need to define it here. 
```
...
ggplot(heatmap_order, aes(x = year, y = occupation, fill = total_earnings)) +
  ...
  scale_y_discrete(labels = scales::label_wrap(30)) + # wraps y-axis labels for long names
  guides(fill = guide_colorbar(barwidth = 15, barheight = 0.75, title.position = "top")) + # for the legend colorbar, move colorbar title to top too 
  theme_minimal() +
  theme(
    legend.position = "top", # move colorbar to top
...
ggplot(subset_jobs) +
  ...
  # facet wrap by group ----
  facet_wrap(~group_label, nrow = 3, scales = "free_y") + # "free_y" plots only the axis labels that exist in each group. Frees the y axis
