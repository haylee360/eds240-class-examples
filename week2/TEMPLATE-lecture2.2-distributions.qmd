...
mko_clean %>% 
  ...
  ggplot(aes(Temp_bot, fill = month_name)) +
...
mko_clean |> 
  filter(month_name %in% c("April", "June", "October")) |> 
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  geom_density(alpha = 0.5, adjust = 0.5) + # higher adjust, more smoothing
  scale_fill_manual(values = c("#2C5374", "#ADD8E6", "#8B3A3A"))
...
ggplot(mko_clean, aes(x = Temp_bot, 
                      y = month_name, 
                      fill = after_stat(x))) + # compute based on x-axis var. Compute color values after the statistical transformation from ridges gradient
  ggridges::geom_density_ridges_gradient(rel_min_height = 0.01, # chops tails off where color falls off
                                         scale = 3) +  # space between curves. 3 crunches them together more
  scale_fill_gradientn(colors = c("#2C5374","#849BB4", "#D9E7EC", "#EF8080", "#8B3A3A")) +
  scale_y_discrete(limits = rev(month.name)) # rev reverses order
```
...
ggplot(mko_clean, aes(month_name, Temp_bot)) +
  ...
  coord_flip() # flips axes without having to change aes
```
...
# boxplot + beeswarm ----
ggplot(penguins, aes(x = species, y = body_mass_g)) +
  geom_boxplot(outlier.shape = NA) + # removes outliers to avoid repeated info with jitter
