...
# If API is down
# us_state_pop <- read_csv(here("week2", "data", "us_state_pop.csv"))
...
#| echo: true
# line chart ----
ggplot(lyme_pop, aes(year, cases_per100k, group = state)) + # group by state
  geom_line() +
  geom_point()
```
...
lyme_pop |> 
  ...
  geom_line() +
  scale_y_continuous(breaks = seq(0, 190, by = 1)) +
  coord_fixed(ratio = 1) # aspect ratio. x axis units same length as y axis units
```
...
lyme_pop |> 
  ...
  geom_area() +
  scale_x_discrete(expand = c(0, NA)) # start x at 0, doesn't care about y
```
